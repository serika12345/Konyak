...
work_root="${KONYAK_MACOS_PROFILE_INSTALL_SMOKE_WORK_ROOT:-$repo_root/.dart_tool/konyak/macos-profile-install-cli-smoke}"
...
if [[ "$resolved_work_root" == "/" ]]; then
  echo "Profile-install smoke work root must not resolve to the filesystem root." >&2
  exit 64
fi
...
rm -rf "$data_home" "$config_home" "$home_dir" "$launcher_home" \
  "$resource_cache" "$logs_dir" "$certificate_dir"
mkdir -p "$logs_dir" "$certificate_dir" "$runtime_root:h" "$home_dir" \
  "$launcher_home"
...
run_cli_success terminate-fixture-processes "$command_timeout" \
