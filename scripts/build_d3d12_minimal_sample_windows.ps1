...
if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
  throw "The D3D12 minimal sample must be built on Windows."
}

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$fixtureRoot = Join-Path $repoRoot "tests/fixtures/windows/d3d12_minimal_sample"
$artifactRoot = Join-Path $repoRoot ".dart_tool/konyak/windows-d3d12-fixture"
...
$builtExe = Join-Path $fixtureRoot "out/build/$preset/Release/konyak_d3d12_minimal.exe"
if (-not (Test-Path $builtExe)) {
  throw "Build completed without producing $builtExe"
}

New-Item -ItemType Directory -Force -Path $artifactRoot | Out-Null
$artifact = Join-Path $artifactRoot "konyak_d3d12_minimal.exe"
